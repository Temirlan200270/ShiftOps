...
поле `job_title` у `users` (nullable строка, уже в контексте `organization_id`)
для подписи в UI («Шеф-повар» при системной роли `admin`); права по-прежнему
определяет только `user_role`.
