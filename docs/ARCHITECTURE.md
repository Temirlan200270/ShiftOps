...
- **Последствия:** оффлайн-очередь Service Worker'а — best-effort (сейчас
  в первую очередь задачи и вложения; закрытие смены с `delay_reason` в
  очереди — целевое поведение, см. `UX_FLOW.md` EC-1b); нативное
  Flutter-приложение запланировано на V3.
