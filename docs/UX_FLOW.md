...
- TWA держит фото в IndexedDB через простую очередь (см. `apps/web/lib/offline/queue.ts`).
...
- Закрытие смены работает **offline-first** через очередь в IndexedDB
  (см. `apps/web/lib/offline/close-queue.ts`), по паттерну очереди фото.
- Если связи нет в момент «Закрыть смену», приложение сохраняет закрытие
  (включая `delay_reason` и `violation_reason`) и показывает подсказку,
  что действие будет отправлено автоматически при появлении сети.
- При reconnect / focus очередь повторяет запрос закрытия с тем же телом.
  При успехе показывается тост «Смена закрыта», а открытый экран
  переходит к сводке (summary).
- Если сервер отвечает ошибкой, которую нельзя исправить повтором
  (например, смена уже закрыта другим способом), запись удаляется из
  очереди без повторных попыток.
