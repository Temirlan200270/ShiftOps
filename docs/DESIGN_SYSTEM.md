...
### Запрет на IT‑термины в пользовательском тексте (строго)

Любой текст, который видит пользователь (в TWA или в Telegram‑сообщениях),
пишется **человеческим языком**.

- **Нельзя**: “API”, “JWT”, “RLS”, “Redis”, “WebSocket”, коды (`401`, `5xx`),
  названия эндпоинтов (`/v1/...`), внутренние поля/статусы (`actual_start`,
  `ShiftStatus`, “payload”, “cron”).
- **Можно**: “интернет”, “смена”, “задачи”, “фото”, “попробуйте ещё раз”,
  “откройте приложение заново”.
- **Ошибки/сбои**: всегда давать следующий шаг (“проверьте интернет” /
  “повторите позже”), без диагностики технологий.
