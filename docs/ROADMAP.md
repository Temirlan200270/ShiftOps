...
- **Аналитика:** разрез по постам (`station_label` / `slot_index`) в
  `/v1/analytics/overview` — ✅ уже реализовано (карточка/агрегация по постам).
...
- **Эфир:** проактивный TG-пинг в админ-чат по вакантным слотам в зоне
  риска (дополнение к списку в TWA) — ✅ реализовано (админ-чат + зеркало owner DM).
- **Оффлайн:** очередь на `POST .../close` с `delay_reason` (паритет с
  очередью задач) — ✅ реализовано (очередь закрытия смены в IndexedDB; см. `UX_FLOW.md` EC-1b).
