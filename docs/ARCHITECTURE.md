...
2. Фронтенд POST'ит на `/api/v1/auth/telegram` сырую строку.
3. Бэкенд HMAC'ит её с `BOT_TOKEN`, валидирует `auth_date`, ищет /
   создаёт пользователя, выпускает JWT (access 15 мин, refresh 7 дней).
4. Refresh-токен лежит в cookie `httpOnly; SameSite=Strict; Secure` со
   scope в origin API.
