...
### Операционные выгрузки (владелец / аудит)

- Не путать с GDPR-экспортом: **ежедневная** выгрузка закрытых смен и
  оценок за **предыдущий календарный день** (см. `PRD.md` §5.11) —
  продуктовый сценарий для собственника (CSV/PDF, возможна доставка в
  Telegram / email по политике орг.). Хранение и доступ к файлам — те же
  принципы least privilege; логирование факта выгрузки в `audit_events`
  при появлении автоматизации.
