...
Полный релиз (API + Vercel prod + Sentry) — по git-тегу `v*` или вручную в Actions:
```bash
git tag -a v0.1.0 -m "MVP pilot"
git push origin v0.1.0
```

Пуш в `main`, если меняется `apps/api/**`, запускает тот же `deploy.yml`, но **только Fly** (миграции через `release_command` + webhook); фронт на Vercel по-прежнему уезжает отдельно (`vercel-web.yml`).

Миграции выполняются автоматически через `release_command` в `fly.toml` при каждом таком деплое API.
